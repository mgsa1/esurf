 ...
 *
 *  'radial': z = A · cos(k·r − ω·t)   r = √((x−ox)² + (y−oy)²)
 *  'planar': z = A · cos(k·(cos θ·x + sin θ·y) − ω·t)
 *
...
export function wave2Z(worldX: number, worldY: number, params: WaveParams, t: number, alpha?: number): number {
  ...
  if (params.wave2Mode === 'planar') {
    const dx = Math.cos(params.wave2Direction);
    const dy = Math.sin(params.wave2Direction);
    const along = dx * worldX + dy * worldY;
    return params.wave2Amplitude * eff * Math.cos(k2 * along - omega2 * tCycled2);
  }
...
/**
 * Height contribution from wave 1.
 *
 * 'radial' mode: z1 = A · cos(k·r − ω·t)   r = √(x²+y²)
 * 'planar' mode: z1 = A · cos(k·(cos θ·x + sin θ·y) − ω·t)
 *
 * Planar mode gives parallel crests rolling in the wave1Direction — the swell
 * model used for surfing. Radial mode is the drop-a-stone-in-a-pond ripple.
 */
export function wave1Z(worldX: number, worldY: number, params: WaveParams, t: number): number {
  const { k, omega, phase } = derived(params, t);
  if (params.wave1Mode === 'planar') {
    const dx = Math.cos(params.wave1Direction);
    const dy = Math.sin(params.wave1Direction);
    const along = dx * worldX + dy * worldY;
    return params.amplitude * Math.cos(k * along - omega * phase);
  }
  const r = Math.sqrt(worldX * worldX + worldY * worldY);
  return params.amplitude * Math.cos(k * r - omega * phase);
}
...
export function surfaceZ(worldX: number, worldY: number, params: WaveParams, t: number, wave2Alpha?: number): number {
  return wave1Z(worldX, worldY, params, t) + wave2Z(worldX, worldY, params, t, wave2Alpha);
}
